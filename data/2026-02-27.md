...
TL;DR: JWST观测HD 15337 c（一颗亚海王星）的3-5 μm透射光谱，发现光谱无特征，表明高金属丰度和/或高空气溶胶，并检测到显著的凌星时间变化。


<details>
  <summary>Details</summary>
Motivation: 研究亚海王星HD 15337 c的大气特性，利用JWST的高精度观测能力探索这类行星的大气组成和结构，同时验证JWST在测量凌星时间变化方面的能力。

Method: 使用JWST COMPASS项目观测HD 15337 c的3-5 μm透射光谱，采用两个独立的数据处理流程进行数据还原，进行热化学平衡反演分析大气成分，结合TESS和CHEOPS数据测量凌星时间变化。

Result: 获得高精度透射光谱（30-70 ppm精度），光谱无特征，可用阶跃函数描述；检测到>20分钟的凌星时间变化，暗示存在外部伴星；大气反演排除了金属丰度<310-600倍太阳值的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 研究半径谷行星TOI-260 b的大气成分，这类行星的大气性质尚不明确，需要更多观测来理解其多样性

Method: 使用JWST/NIRSpec G395H光谱仪进行两次凌星观测，结合数据获得26 ppm精度的透射光谱，采用30像素宽(R~200)的光谱分箱方案，使用非参数模型评估特征显著性，运行PLATON反演分析大气成分

Result: 透射光谱基本无特征，仅在3.17 μm附近可能存在特征；未检测到清晰的分子特征，但约束大气金属丰度大于200倍太阳丰度（假设不透明层压力≲2.5 mbar）；3.17 μm特征可能源于系统误差而非天体物理起源

Conclusion: TOI-260 b是半径谷行星样本中的一员，这类行星已显示出大气成分的多样性；确定这些神秘行星的真实性质需要更多的望远镜观测时间投入
...
TL;DR: PICASO 4.0是开源大气建模包的重大更新，专门用于模拟褐矮星和系外行星富氢大气的热结构和光谱，新增了自洽云建模、挥发物雨落/冷阱处理、光化学支持以及参数化能量注入方案。


<details>
  <summary>Details</summary>
Motivation: 该更新的主要动机源于JWST观测揭示的非平衡化学和云层普遍存在现象，需要更先进的建模工具来准确描述这些复杂大气过程。

Method: 在现有辐射-对流平衡模型框架基础上，整合了Virga进行自洽云建模，增加了挥发物雨落和冷阱的灵活处理方法，支持光化学过程，并引入了参数化能量注入方案来模拟额外外部或内部加热。

Result: 新功能通过多个基准测试验证，包括与Sonora Diamondback网格、能量注入大气、斑块云模型以及WASP-39b光化学模型的对比，展示了模型的可靠性和准确性。

Conclusion: PICASO 4.0作为一个持续开发的开源包，旨在为所有亚恒星天体提供可重复、社区驱动的大气建模能力，适应现代观测需求。
...
<details>
  <summary>Details</summary>
Motivation: 行星形成理论预测存在自由漂浮行星（FFPs），这些行星从形成位置被抛射出来。由于它们发出的光极少，需要通过微引力透镜效应来探测。本研究旨在通过分析微引力透镜事件来发现和表征自由漂浮行星候选体。

Method: 利用KMT微引力透镜巡天数据，分析事件KMT-2024-BLG-3237的光变曲线。该光变曲线显示出强烈的有限源效应特征：振幅小（≲0.9星等）且时标短（≲3天）。通过拟合得到爱因斯坦时标和角爱因斯坦半径，进而估算透镜质量。采用贝叶斯分析确定透镜质量和距离。

Result: 测得爱因斯坦时标t_E = 0.54±0.02天，角爱因斯坦半径θ_E = 6.30±0.48微角秒。透镜质量估计为M ≃ 102 M_⊕(π_rel/16 μas)^{-1}，具体取决于未知的相对视差π_rel。贝叶斯分析给出透镜质量M = 67.3^{+103.2}_{-42.5} M_⊕，距离D_L = 7.34^{+0.96}_{-2.11}千秒差距。这是第11个测得θ_E < 10微角秒的孤立微引力透镜。对透镜宿主星的可能特征进行额外搜索未发现显著证据。

Conclusion: KMT-2024-BLG-3237事件中发现了一个自由漂浮行星候选体，其质量约为67地球质量，距离约7.34千秒差距。该发现支持行星形成理论中自由漂浮行星存在的预测，并展示了微引力透镜技术在探测这类暗弱天体方面的有效性。这是第11个测得小角爱因斯坦半径的孤立微引力透镜事件，进一步丰富了自由漂浮行星的观测样本。
...
TL;DR: 利用Gaia DR3小行星反射光谱进行监督概率分类，通过主成分分析和核密度估计建立分类模型，验证效果良好，并应用于主要碰撞家族分析。


<details>
  <summary>Details</summary>
Motivation: 利用Gaia DR3提供的高质量小行星反射光谱数据，建立系统性的概率分类方法，以研究主带小行星的成分结构，识别家族侵入体，并将小行星群体与陨石群联系起来。

Method: 使用Gaia DR3高质量光谱和文献中具有稳健光谱分类的参考集，构建主成分表示。对每个主要光谱复合体（C、S、X）和末端成员类别（B、D、A、L、K、V），使用多元核密度估计在PC空间中建模参考对象的分布，得到每个光谱类别的似然和分类置信度的定量度量。

Result: 验证显示对具有独特反射特征的类型（S复合体、D、V、A型）分类性能良好。光谱连续类别（B-C复合体、K-L-S复合体、X复合体）由于Gaia分光光度法的有限波长范围而显示出预期的混合程度。对六个主要小行星碰撞家族的分析显示与地基光谱学结果高度一致，并在Flora和Eunomia家族中检测到更多富橄榄石的A型物质，对Tirela家族的光谱多样性提供了新见解。

Conclusion: 该研究为完整Gaia DR3小行星样本提供了完全概率分类目录，为研究主带成分结构、识别家族侵入体、连接小行星与陨石群提供了资源，并为未来Gaia数据发布（特别是预计2026年底发布的DR4）建立了方法学框架。
...
TL;DR: 研究评估ELT/ANDES高分辨率交叉相关光谱技术对长周期亚海王星大气探测的能力，发现包含凌外曝光可显著增强信号探测，但需要正确的前处理传播。


<details>
  <summary>Details</summary>
Motivation: 随着下一代设施如ELT/ANDES的发展，高分辨率交叉相关光谱技术将扩展到更小、更长周期的行星。但长周期行星的缓慢多普勒漂移会增加其信号与固定地球大气污染分离的难度，增加信号在去趋势过程中被移除的风险。因此需要量化包含凌外曝光对信号恢复和参数估计的影响。

Method: 使用Ratri管道模拟YJH波段的透射观测，并用HRCCS框架Upamana进行分析。针对V1298Taub注入与HST、Spitzer和JWST约束一致的大气信号；针对TOI-451c探索从亚太阳到超太阳C/O比的模型以评估成分敏感性。

Result: 包含凌外曝光可显著增强探测能力，但前提是去趋势效应必须在交叉相关前一致地传播到模板模型中。没有这种再处理步骤，恢复的轨道参数会显著偏离注入值或导致探测显著性降低。对于V1298Tau b，在约10小时内可实现H2O、H2S和CO的>5σ探测；对于TOI-451c，区分亚太阳和超太阳C/O比需要≥18小时。

Conclusion: ELT/ANDES的高分辨率交叉相关光谱技术将成为探测ELT时代年轻、长周期亚海王星大气多样性的关键工具，但需要正确处理包含凌外曝光的数据以确保准确的参数恢复。
...
TL;DR: 利用LAMOST光谱数据结合Gaia和红外测光，在150 pc内的FGK星中系统搜寻暖碎片盘候选体，识别出12个候选体（10个新发现），建立了可靠的小样本用于碎片盘演化研究。


<details>
  <summary>Details</summary>
Motivation: 系统搜寻主序星周围的暖碎片盘，以追踪晚期类地行星形成过程，需要建立基于均匀光谱数据的可靠候选样本。

Method: 结合LAMOST DR12光谱选择FGK星、Gaia天体测量和多波段红外测光，通过SED拟合识别红外超量，并进行保守的逐源验证，最终筛选出碎片盘候选体。

Result: 获得12个碎片盘候选体（10个新发现），宿主恒星年龄多在数十亿年，NEOWISE监测显示无显著W1/W2波段变化，Gaia DR3仅发现两个候选体有遥远伴星，三个候选体在W3和W4波段均显示超量辐射。

Conclusion: 建立了基于均匀LAMOST光谱的小而可靠的碎片盘候选样本，为未来研究碎片盘演化和恒星活动提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 凌星系外行星是唯一能够高精度测量物理和轨道特性的系外行星，对理解行星特征和形成过程具有重要价值。随着已知凌星行星数量不断增加，需要为科学界维护一个系统性的数据库来整合这些宝贵数据。

Method: TEPCat作为一个在线数据库，系统性地收集和整理已知凌星行星系统的物理和观测特性数据，包括行星分类、光变曲线中的食特征等，并通过网站提供免费访问。

Result: 建立了TEPCat数据库，包含已知凌星行星系统的全面物理和观测特性数据，提供了示例图表展示有趣量值，并讨论了行星分类和光变曲线食特征。

Conclusion: TEPCat是一个重要的科学资源，为研究凌星系外行星提供了系统化的数据支持，有助于推动系外行星研究领域的发展。该数据库在线维护并免费提供访问。
...
TL;DR: 通过N体模拟研究行星半径与偏心率关系，发现半径谷附近的偏心率峰值反映了原行星质量分布的二重性，支持迷你海王星可能是水世界的观点


<details>
  <summary>Details</summary>
Motivation: 解决超级地球和迷你海王星是否具有共同岩石成分（单一群体）还是由岩石行星和冰质行星（两个群体）组成的争议，为半径谷的形成机制提供诊断依据

Method: 使用N体模拟研究近距离行星的半径和偏心率如何依赖于其原行星的质量和轨道构型，分析最终行星偏心率与系统初始Safronov数的关系

Result: 在双群体系统中，岩石和相对更重的冰质原行星之间的能量均分导致两组间强烈的偏心率对比，在半径谷附近形成峰值；单一岩石行星系统（无论有无富氢大气）不出现此特征

Conclusion: 偏心率-半径关系反映了原行星质量分布的二重性（可能源于不同盘位置的形成），表明相当一部分迷你海王星是水世界，观测到的分布可能是纯岩石系统、冰质主导系统和混合系统的组合
...
### [10] [A Multi-Diagnostic Observational Framework for Magnetosonic Solitary Waves During Geomagnetic Storms in Solar Cycles 24 and 25 using Cluster II Mission](https://arxiv.org/abs/2602.22614)
...
<details>
  <summary>Details</summary>
Motivation: 虽然孤子结构在多种空间等离子体环境中被广泛报道，但在地磁暴期间的观测仍然有限。地磁暴是由太阳风与地球磁层相互作用驱动的大尺度扰动，研究孤子在这些极端条件下的特征对于理解空间等离子体物理和地磁暴演化机制具有重要意义。

Method: 使用Cluster II任务的高分辨率原位磁场测量数据，系统性地研究了有利于孤子产生的等离子体条件及其在暴时动力学中的演化。开发了一个综合的多诊断观测框架，结合多种先进的分析技术，可靠地检测和表征磁声孤子。

Result: 研究结果表明，两次地磁暴中的孤子结构主要出现在暴前阶段（主相之前），这表明它们可能作为增强地磁活动的潜在前兆信号。研究还详细描述了孤子在这些极端条件下的特征和演化规律。

Conclusion: 磁声孤子在地磁暴期间主要出现在暴前阶段，可能作为地磁活动增强的潜在前兆。这一发现为理解空间等离子体非线性过程和地磁暴演化机制提供了新的见解，并强调了孤子监测在空间天气预报中的潜在应用价值。
...
### [11] [JWST-DECO: Temporal Variations in the Mid-IR Silicate Features of Two T Tauri Discs Based on Spitzer and JWST observations](https://arxiv.org/abs/2602.22615)
...
TL;DR: JWST观测发现行星形成盘中的硅酸盐发射谱在20年间显著变化，支持晶体硅酸盐从高温内盘向外传输到较冷区域的模型。


<details>
  <summary>Details</summary>
Motivation: 研究行星形成盘中硅酸盐发射谱的时间变化，以了解盘内结构演化和矿物传输过程。

Method: 使用JWST观测Sz 96和IP Tau两个原行星盘，与20年前的Spitzer观测数据对比，分析硅酸盐发射谱的变化特征。

Result: 两个源的硅酸盐发射谱均发生显著变化：Sz 96在≤18μm波段变暗，IP Tau在整个波长范围变亮且在10μm附近增强；JWST数据显示晶体硅酸盐温度低于非晶硅酸盐。

Conclusion: 观测到的谱变化源于盘内结构变化，晶体硅酸盐在高温内盘形成后向外传输到较冷区域，JWST的高光谱分辨率能更清晰识别晶体特征。
...
### [12] [Detection of circumstellar disks around nearby young brown dwarf candidates](https://arxiv.org/abs/2602.22832)
*N. Phan-Bao,M. S. Bessell,E. L. Martín*

Main category: astro-ph.SR

TL;DR: 在3928个候选天体中搜索年轻晚M型和褐矮星周围的原行星盘，发现23个有盘天体（10个新发现），其中21个为褐矮星候选体，2个为极低质量恒星，并检测到锂吸收线和吸积活动。


<details>
  <summary>Details</summary>
Motivation: 研究邻近年轻星协中晚M型和褐矮星周围的原行星盘，有助于理解这些极低质量天体的形成机制以及它们周围类地行星的形成过程。同时，发现新的极低质量年轻星协成员能显著改善对年轻星协的现有认知。

Method: 使用DENIS、2MASS和WISE的光度数据结合Gaia的三角视差，构建所有候选体的光谱能量分布以探测指示原行星盘存在的红外超量辐射。随后对候选体进行光谱观测，通过锂吸收线确认其年轻性，并利用Hα发射线探测吸积活动。

Result: 在3928个候选体中检测到23个拥有原行星盘的天体：10个为新发现，13个为文献已报道。质量估算表明21个为褐矮星候选体，2个为极低质量恒星。DENIS J0534552-104808距离238pc，可能是猎户座D云前景星群的首个褐矮星候选成员。光谱观测在11个候选体中检测到锂，识别出7个吸积体和1个潜在吸积体。

Conclusion: 成功发现并确认了年轻极低质量天体周围的原行星盘系统，其中DENIS-P J0500245-333042（年龄20Myr的褐矮星候选体）检测到的强烈持久吸积活动，为极低质量天体周围类地行星的形成提供了额外证据支持。
...
### [13] [Eating planets makes you younger: The magnetic dynamo rejuvenation of GJ 504 by planetary engulfment](https://arxiv.org/abs/2602.22979)
*S. Bellotti,C. Pezzotti,G. Buldgen,A. A. Vidotto,D. Evensberget,E. Magaudda*

Main category: astro-ph.SR

TL;DR: GJ 504是一颗演化程度较高（20亿年）的类太阳恒星，其快速自转（3.4天）和强烈磁活动与演化阶段不符。研究通过分析其大尺度磁场和磁化星风，发现行星吞噬可能是解释其异常旋转的关键机制。


<details>
  <summary>Details</summary>
Motivation: GJ 504作为演化恒星却具有快速自转和强烈磁活动，这与恒星演化理论相矛盾。研究旨在通过表征其大尺度磁场和磁化星风，为测试"近距离木星质量行星被恒星潮汐作用推入恒星内部，导致恒星自转加速和磁发电机年轻化"这一假设提供观测约束。

Method: 使用ESPaDOnS收集的GJ 504光谱偏振观测数据，通过Zeeman-Doppler成像技术重建大尺度磁场结构。将磁场信息输入3D MHD模拟，模拟恒星风和空间环境，从而约束风驱动的角动量损失率。最后将角动量损失率与两种情景下的旋转演化轨迹进行比较：有/无近距离木星质量伴星被吞噬的情况。

Result: 发现GJ 504具有大尺度、偶极、非轴对称磁场，平均强度5.3 G，与演化早期G型星相似。通过MHD模拟获得角动量损失率约束。比较两种演化情景后，只有行星吞噬情景能够解释先前文献中的观测约束（恒星自转、X射线光度）以及本研究推导并重新标定的角动量损失率。研究还识别出HD 75332作为另一个行星吞噬候选体。

Conclusion: 行星吞噬是解释GJ 504异常快速自转和强烈磁活动的最可能机制。虽然许多具有相似质量和自转周期的恒星不需要行星吞噬来解释其演化，但GJ 504和HD 75332的发现表明行星吞噬可能不是孤立现象，而是某些恒星异常演化的重要机制。
