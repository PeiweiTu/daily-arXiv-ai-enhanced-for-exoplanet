...
### [1] [Insights into the Exoplanet Radius Valley from Host-Star Ages, Activity, Chemistry, and Birth Radius](https://arxiv.org/abs/2603.21581)
*Xunzhou Chen,Tiancheng Sun,Yuxi,Lu,Zixuan Lu,Lifei Ye*

Main category: astro-ph.SR

TL;DR: 研究通过769颗系外行星数据分析半径谷特征，发现其随恒星年龄、磁活动、金属丰度和银河系诞生环境演化，支持核心驱动大气质量损失模型而非单纯光致蒸发


<details>
  <summary>Details</summary>
Motivation: 半径谷是近距离小质量系外行星尺寸分布的双峰特征，被广泛解释为大气损失的标志。研究旨在探究半径谷对宿主恒星特性的依赖性，以约束行星形成和大气演化过程

Method: 使用769颗围绕558颗恒星的行星样本，推导恒星年龄、色球活动、银河系诞生半径和元素丰度，分析半径谷随这些参数的变化规律

Result: 1) 半径谷在~3 Gyr年龄时未完全建立，在Gyr时间尺度上演化；2) 对磁活动的依赖非单调：磁宁静恒星存在清晰谷，高活动系统未显示系统性更强耗尽；3) 形态随金属丰度变化：贫金属恒星最强，近太阳金属丰度减弱，最高金属丰度部分增强；4) 对[Mg/Si]等难熔元素比敏感，与[C/O]相关性较弱；5) 半径谷依赖银河系诞生环境，太阳诞生半径附近系统显示高比例类地行星和清晰双峰结构

Conclusion: 观测结果更支持核心驱动大气质量损失模型而非单纯光致蒸发。太阳系形成于具有良好发育的地球大小行星种群的区域
...
### [2] [Diffuse and specular brightness models applied to LEO satellites. Case study: The ONEWEB constellation](https://arxiv.org/abs/2603.21859)
...
TL;DR: 研究分析了300多颗ONEWEB卫星的反射特性，发现现有亮度模型无法完全解释观测到的相位角相关亮度调制，需要更复杂的模型考虑卫星实际形状、姿态变化和地球反照率影响。


<details>
  <summary>Details</summary>
Motivation: 为了更好地理解低地球轨道卫星的观测亮度，需要表征其反射特性，而反射特性主要取决于卫星平台设计。通过分析ONEWEB卫星的反射率，可以改进卫星亮度预测模型。

Method: 使用天体测量和光度测量数据，结合双行轨道根数，计算视星等和距离星等，以及太阳、卫星和观测者之间的相关角度。采用差分进化蒙特卡洛算法拟合三种反射模型参数：漫反射朗伯球体模型、漫反射和镜面反射朗伯球体模型、相对反射亮度模型。

Result: 所有模型都能拟合卫星的平均观测亮度，但无法描述观测到的相位角相关亮度调制。所有情况下的残差标准差约为0.6星等，而观测光度误差平均约为0.2星等。

Conclusion: 研究的亮度模型仅依赖于太阳-物体-观测者的相对位置，而不考虑反射体表面相对于观测者的具体方向，无法完全解释ONEWEB星座卫星的观测亮度。需要考虑卫星的实际形状、变化的姿态以及地球反照率的影响，才能更好地解释卫星光度观测结果。
...
### [3] [Theoretical Ion Sputtering Yields from Loose Powders using a Multiscale Monte Carlo Approach](https://arxiv.org/abs/2603.20251)
...
TL;DR: 开发多尺度蒙特卡洛模型模拟粉末溅射，发现松散粉末溅射与平面或粗糙表面显著不同，主要差异源于粉末内部空隙结构，并推导出两个拟合函数用于预测粉末溅射产额和角分布。


<details>
  <summary>Details</summary>
Motivation: 离子溅射在行星科学和工业中具有重要性，但松散粉末的溅射过程仍缺乏深入理解，需要建立更精确的模型来模拟粉末溅射特性。

Method: 开发多尺度蒙特卡洛模型模拟粉末溅射，采用比体素方法更高保真度的靶材几何结构，模拟Kr+离子轰击Cu粉末和平板，分析溅射产额和角分布差异。

Result: 发现松散粉末溅射与平板或粗糙表面存在四个主要差异：1) 所有离子能量下，溅射产额由向后喷射主导；2) 类似无大气天体光学观测中的对向效应；3) 角分布峰值仅为平板一半或更少；4) 离子能量增加时，喷射角分布无初级到次级碰撞演化。这些行为归因于粉末内部空隙结构。

Conclusion: 推导出两个拟合函数：1) 将粉末逃逸溅射产额与平面表面产额关联，仅依赖孔隙率、入射角、平均局部入射角和平板产额；2) 提供孔隙率>0.49时逃逸喷射的双微分角分布。这些函数为松散粉末的绝对双微分逃逸溅射产额提供了潜在的通用拟合函数。
...
### [4] [Radiating Bondi Flows I: Dimensionless Framework and Constant Opacity Solutions](https://arxiv.org/abs/2603.20373)
*Avery Bailey,Andrew Youdin,Kaitlin Kratter*

Main category: astro-ph.EP

TL;DR: 扩展Bondi吸积模型以包含辐射反馈效应，研究气体压力主导环境中辐射加热和冷却对稳态球对称吸积的影响，发现辐射反馈主要起抑制作用。


<details>
  <summary>Details</summary>
Motivation: Bondi (1952)的经典吸积理论未考虑辐射反馈效应，而在气体压力主导的环境中（如行星形成过程），辐射加热和冷却对吸积过程有重要影响，需要建立包含这些效应的理论模型。

Method: 构建包含辐射加热和冷却的稳态球对称吸积解，假设恒定不透明度，用四个无量纲参数控制：绝热指数γ、邦迪半径光学深度τ_B、无穷远处无量纲光度L̃_∞、特征无量纲冷却时间β。在参数空间(τ_B, L̃_∞, β)∈[10^{-3}, 10^3]内进行数值求解。

Result: 与辐射压力主导环境不同，辐射反馈主要抑制吸积，特别是在高τ_B、高L̃_∞和/或高β时。获得解析描述确认反馈的抑制性质，给出吸积率标度关系：大L̃_∞时Ṁ∼L̃_∞^{-5/4}，大τ_B时Ṁ∼τ_B^{-10/11}β^{-5/11}，大L̃_∞τ_B时Ṁ∼(L̃_∞τ_B)^{-5/8}。

Conclusion: 辐射反馈在气体压力主导环境中主要起抑制作用，这对行星形成等过程具有重要意义，其中辐射加热显著但系统仍处于气体压力主导状态。还讨论了对流在这些稳态解中的潜在作用。
...
### [5] [Radiating Bondi Flows II: Giant Planet Accretion Models](https://arxiv.org/abs/2603.20377)
*Avery Bailey,Kaitlin Kratter,Andrew Youdin*

Main category: astro-ph.EP

TL;DR: 该研究量化了巨行星形成过程中辐射反馈对气体吸积的抑制效应，发现辐射加热可使吸积率降低1-2个数量级，并提供了开源工具用于现有模型整合。


<details>
  <summary>Details</summary>
Motivation: 传统巨行星形成模型基于绝热状态方程，忽略了行星及其吸积激波产生的辐射反馈对吸积流的加热效应，这可能导致对吸积率的高估。

Method: 在系列论文第一篇的基础上，量化实际行星形成条件下的辐射反馈效应，研究不同参数（尘埃不透明度、激波效率、行星半径）的影响，并进行全时变模拟验证熵剖面的稳定性。

Result: 对于未扰动盘中的行星形成，在~10 AU内辐射反馈使吸积抑制1-2个数量级；对于开缝行星，抑制效应约为1个数量级。该机制对尘埃不透明度、激波效率和行星半径等参数相对不敏感，且相关熵剖面稳定、不易对流。

Conclusion: 辐射反馈显著抑制巨行星形成过程中的气体吸积，传统Bondi吸积率参数化高估了实际吸积效率。研究提供了开源工具，便于将这一物理机制整合到现有吸积模型和种群合成框架中。
...
### [6] [Isotopic Signature of Organic Molecules from Beyond the Solar System: An Enriched Methane D/H Ratio in the Interstellar Object 3I/ATLAS](https://arxiv.org/abs/2603.20445)
...
TL;DR: 利用詹姆斯·韦伯太空望远镜测量星际天体3I/ATLAS彗发中甲烷的氘氢比(D/H)高达(3.31±0.34)%，比67P彗星高14倍，表明其形成于与太阳系完全不同的高氘丰度原始环境中。


<details>
  <summary>Details</summary>
Motivation: 研究星际天体的挥发性成分可以揭示其母恒星系统的行星形成环境。通过测量星际天体3I/ATLAS的氘氢比，可以了解其形成环境的物理化学条件，并与太阳系彗星进行比较，探索不同恒星系统行星形成的多样性。

Method: 使用詹姆斯·韦伯太空望远镜对星际天体3I/ATLAS的彗发进行近红外光谱观测，测量其中甲烷(CH₃D)的氘氢比(D/H)，并与罗塞塔号探测器测量的67P/Churyumov-Gerasimenko彗星数据进行对比分析。

Result: 测得3I/ATLAS中甲烷的D/H比为(3.31±0.34)%，比67P彗星高14±2倍，是迄今在星际天体中检测到氘代有机分子的罕见案例。甲烷与水之间的氘化比例在两种彗星中在1.2σ范围内一致，表明氘富集具有系统性特征。

Conclusion: 3I/ATLAS的高氘氢比表明其形成于一个高氘丰度的原始环境中，这种环境由原行星盘的局部低温条件和前身星际云的高氘元素比共同造成。该天体形成环境与太阳系起源环境截然不同，为研究系外行星系统形成提供了重要线索。
...
### [7] [Coma Physics of an Interstellar Object: JWST Spatial-Spectral Mapping of 3I/ATLAS](https://arxiv.org/abs/2603.20460)
...
TL;DR: JWST观测星际天体3I/ATLAS的彗发分子发射，测量了CO、CO₂、H₂O、CH₃OH和CH₄的丰度分布，揭示了冰核中挥发物的分馏机制和彗发加热冷却过程。


<details>
  <summary>Details</summary>
Motivation: 研究星际天体3I/ATLAS在远离太阳进入水冰线区域时的分子化学特性，了解其冰核中挥发物的分馏机制和彗发物理过程。

Method: 使用詹姆斯·韦伯太空望远镜（JWST）于2025年12月22-23日观测3I/ATLAS，在日心距离2.37-2.41天文单位处测量CO、CO₂、H₂O、CH₃OH和CH₄的分子发射，生成柱密度和转动温度的空间-光谱分布图。

Result: CO是最丰富的分子，H₂O和CO₂相对于CO的丰度分别为(44.4±0.7)%和(42.4±0.9)%。非极性分子在彗发中的空间分布高度各向异性，而极性分子分布更对称。H₂O的正仲比（OPR）为2.7±0.1，略低于平衡值3。

Conclusion: 观测结果揭示了3I/ATLAS冰核中挥发物的分馏机制，表明非极性分子在彗发中经历不同的加热和冷却过程，为理解星际天体的化学演化提供了重要线索。
...
### [8] [A New Method of Measuring Magnetic Field Strength in Highly Structured Protostellar Envelopes](https://arxiv.org/abs/2603.20515)
*Yisheng Tu,Xiaoyuan Yang,Zhi-Yun Li*

Main category: astro-ph.EP

TL;DR: 提出一种基于磁流体动力学动量方程的新方法，用于估算原恒星包层中的垂直和总磁场强度，该方法仅需观测可得的引力加速度和柱密度，并通过模拟校准两个关键无量纲参数。


<details>
  <summary>Details</summary>
Motivation: 磁场在原恒星坍缩和盘形成中起关键作用，但直接测量深埋原恒星包层中的磁场强度仍然困难。现有方法通常需要详细的气体运动学或高分辨率偏振信息，限制了其应用范围。

Method: 从磁流体动力学动量方程推导出磁场强度与两个观测可得的物理量（朝向坍缩中心的投影引力加速度和伪盘的正面柱密度）的关系：|B_z|=(2πa_{b,R}γ_{zR}g_RΣ)^{1/2}。通过非理想磁流体动力学模拟验证假设、证明近似合理性，并校准两个关键无量纲参数a_{b,R}（磁力贡献）和γ_{zR}（磁场几何）。

Result: 模拟验证了方法的假设，校准了两个关键参数并显示其时空变化较弱，使磁场强度估计更加稳健。该方法在湍流和非湍流包层中均适用，且对双极扩散系数不敏感。应用于Class 0源L1157，得到的磁场强度估计与先前独立方法的结果基本一致。

Conclusion: 该方法提供了一种稳健、观测友好的途径来约束深埋原恒星系统中的磁场强度，即使在缺乏详细气体运动学或高分辨率偏振信息的情况下也能应用，为研究原恒星坍缩和盘形成中的磁场作用提供了新工具。
...
### [9] [Pre-perihelion observations of comet P/2023 JN16 (Lemmon) at the 2024 apparition](https://arxiv.org/abs/2603.20900)
*Serhii Borysenko,Alexander Baransky,Christopher A. Onken,Iryna Kulyk*

Main category: astro-ph.EP

TL;DR: 对主带彗星P/2023 JN16的测光分析，估计其核半径小于0.5公里，活动可能由大而慢的颗粒释放驱动


<details>
  <summary>Details</summary>
Motivation: 分析新发现的主带彗星P/2023 JN16的测光特性，研究其尘埃活动特征和物理参数

Method: 使用Siding Spring天文台1.35米SkyMapper望远镜在2024年7-9月期间进行观测，测量r波段星等、Afrho参数和绝对星等

Result: 彗星在长时间内显示尘埃活动，核半径估计小于0.5公里，活动可能由大而慢的颗粒释放驱动

Conclusion: P/2023 JN16是一颗小核主带彗星，其活动机制可能不同于传统彗星，需要进一步研究其尘埃释放过程
...
### [10] [High-resolution spectroscopic atmospheric studies of 5 hot Jupiters across the edge of the Neptune desert](https://arxiv.org/abs/2603.21273)
*Zewen Jiang,Wei Wang,Shumei Jia,Yaqing Shi,Fei Yan,Li Zhou,Qianyi Zou,Xiang Ma,Yue Huang*

Main category: astro-ph.EP

TL;DR: 该研究对五颗系外行星进行大气表征，包括两颗超热木星、两颗经典热木星和一颗热海王星，重点关注大气成分与行星整体性质的关系，发现了不同温度区间大气信号强度的差异。


<details>
  <summary>Details</summary>
Motivation: 热木星（特别是超热木星）因其高温和大尺度高度而成为大气表征的理想目标。研究这五颗行星（包括处于海王星沙漠边界的不同类型行星）可以探索大气成分与行星整体性质之间的联系。

Method: 使用光谱观测技术对五颗行星进行大气研究，包括吸收信号检测和交叉相关技术。对WASP-167b进行了大气反演，确定了多种元素的丰度和平衡温度。

Result: 在三颗密度较高的热木星中未检测到显著吸收信号。在WASP-117b中边缘性检测到Hα和Li I。在WASP-167b中初步检测到Hα和Fe I，并通过交叉相关技术显著检测到Fe I（最大信噪比7.3σ），显示出蓝移信号。反演得到WASP-167b的Fe、Mg、Ca、Ti、V丰度和平衡温度2479K。

Conclusion: 通过比较WASP-173Ab和WASP-167b这两颗超热木星的大气信号差异，提出在平衡温度1900-2300K之间可能存在大气信号的转变，表明行星温度对大气可探测性有重要影响。
...
### [11] [Iron isotope anomalies and the origin of the Earth](https://arxiv.org/abs/2603.21360)
*Timo Hopp,Shengyu Tian,Thorsten Kleine*

Main category: astro-ph.EP

TL;DR: 地球的铁同位素组成与CI球粒陨石存在差异，排除了地球由外太阳系向内漂移的卵石吸积形成的可能性，支持地球物质主要来自内太阳系。


<details>
  <summary>Details</summary>
Motivation: 先前研究基于地球地幔与CI球粒陨石相似的铁同位素组成，提出地球由外太阳系向内漂移的卵石吸积形成。本研究旨在通过更高精度的铁同位素数据验证这一假说。

Method: 使用新的高精度铁同位素数据，特别是包含中子富集的58Fe同位素，对比分析地球地幔与CI球粒陨石的铁同位素组成差异。

Result: 地球地幔与CI球粒陨石在考虑58Fe同位素时显示出明显不同的铁同位素组成，排除了地球地幔铁来自CI球粒陨石类物质的可能性。

Conclusion: 地球的铁主要来自内太阳系，可能由内盘未在陨石中采样的天体输送，支持地球通过内太阳系星子与行星胚胎的层次生长形成的经典模型。
...
### [12] [Modelling solar radial velocities and photometric variability with SOAPv4](https://arxiv.org/abs/2603.21413)
...
TL;DR: 该研究验证了SOAPv4模型在模拟太阳活动区域对径向速度和光度变化影响方面的准确性，通过数学形态学变换识别活动区域，实现了与观测数据的最佳匹配。


<details>
  <summary>Details</summary>
Motivation: 恒星活动是使用径向速度法探测类地行星的主要限制因素。太阳作为唯一表面特征可被空间分辨的恒星，为研究活动区域对径向速度和光度变化的影响提供了独特测试平台。研究旨在验证当前最先进的交叉相关函数模型是否能正确再现观测到的变异性，并评估输入数据和识别技术选择对模拟与观测信号一致性的影响。

Method: 首先使用数学形态学变换对SDO/HMI和AIA图像中的太阳活动区域进行识别，并将此方法与其他最先进的识别方法进行验证。然后使用这些输入运行SOAPv4模型，模拟太阳径向速度和光度时间序列。最后将模拟结果与HARPS-N径向速度观测数据以及VIRGO/SPM光度测量数据进行对比验证。

Result: 使用数学形态学识别方法的模拟结果与观测径向速度时间序列实现了最佳匹配，残差的标准偏差约为0.91 m/s。其他最先进方法产生了更高的填充因子，从而导致更大的差异。光度模拟再现了总体变异性趋势。

Conclusion: 数学形态学变换能够准确识别太阳活动区域。使用这些输入，SOAPv4能够以约0.91 m/s的残差标准偏差再现观测到的太阳径向速度变异性。光度模拟捕捉了总体变异性趋势，证实SOAP能够可靠地模拟黑子和光斑对太阳径向速度和光度的影响。
...
### [13] [The fragility of the Uranian moons during the giant planet instability](https://arxiv.org/abs/2603.21750)
*Matthew S. Clement,Nathan A. Kaib,Andre Izidoro,Rogerio Deienno*

Main category: astro-ph.EP

TL;DR: 研究太阳系巨行星动力学不稳定性期间，木星和天王星规则卫星系统的生存概率，发现两者生存概率均低于15%，且只有一种情况能同时保存两个卫星系统。


<details>
  <summary>Details</summary>
Motivation: 太阳系巨行星形成后经历动力学不稳定性，行星间近距离相遇可能扰动规则卫星系统。需要量化这种不稳定性对木星和天王星卫星系统的影响，特别是卫星能否在行星近距离相遇中幸存。

Method: 使用122个外太阳系动力学历史模拟的相遇历史，建模行星相遇对木星和天王星规则卫星动力学稳定性的影响。分析不同相遇距离（D<0.02 au和D<0.1 au）对卫星系统的影响。

Result: 木星和天王星卫星系统生存概率均低于15%。只有一种情况两者卫星系统能同时幸存。木星卫星在有两个较小额外冰巨行星的不稳定性中更易存活，而天王星卫星在有一个较大额外行星的情况下生存条件更好。近距离相遇（天王星与冰巨行星D<0.02 au，或与气态巨行星D<0.1 au）几乎必然导致卫星系统毁灭。

Conclusion: 太阳系巨行星不稳定性期间，木星和天王星的规则卫星系统很难幸存。天王星卫星可能经历了至少两次重大扰动：导致行星倾斜的撞击和巨行星不稳定性。木星卫星若经历导致碰撞的不稳定性，其拉普拉斯共振可能无法形成。
...
### [14] [New surface composition and temperature maps of Pluto from New Horizons LEISA data](https://arxiv.org/abs/2603.21860)
*A. E. Drozdov,N. V. Emelyanov*

Main category: astro-ph.EP

TL;DR: 该研究通过整合冥王星五个近距离观测数据，创建了7公里/像素的配准等距柱状投影图，联合反演表面温度、冰丰度和颗粒大小，提供了CH4富冰、N2富冰、H2O冰和泰坦索林的空间分布图。


<details>
  <summary>Details</summary>
Motivation: New Horizons RALPH/LEISA近红外光谱可用于冥王星表面冰的区域制图，但扫描间伪影和可变空间分辨率阻碍了定量解释。需要改进数据处理方法以获得更准确的地表成分和物理状态信息。

Method: 将五个近距离观测数据整合为配准的等距柱状投影图（7公里/像素），使用基于Hapke的混合模型联合反演表面温度、冰丰度和颗粒大小。通过每观测的比例和偏移项校正残余扫描间光谱差异，并减轻坏像素和边缘过曝光问题。

Result: 获得了CH4富冰、N2富冰、H2O冰和泰坦索林的空间分布图，以及相应的温度图，提供了冥王星表面成分和物理状态的详细区域信息。

Conclusion: 该方法有效改善了冥王星LEISA数据的定量解释，为理解冥王星表面冰的分布和物理特性提供了更可靠的数据产品，扩展了先前的研究成果。
...
### [15] [Planetary Architectures of Kepler Compact Multis with Binary Star Companions](https://arxiv.org/abs/2603.21897)
*Kendall Sullivan,Gregory J. Gilbert*

Main category: astro-ph.EP

TL;DR: 该研究比较了双星系统和单星系统中行星系统的架构差异，发现双星系统行星具有相似的"豌豆荚"模式，但在周期间距和行星多重性方面存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 双星系统中的行星与单星系统中的行星在统计特征上存在差异，特别是近距离双星系统中行星出现率较低，亚海王星相对出现率被抑制。虽然单星系统行星架构已有深入研究，但双星系统行星架构尚未得到详细调查。

Method: 分析了开普勒行星系统的大样本数据，包括162颗行星分布在118个双星系统，以及880颗行星分布在544个单星系统。比较了这些系统作为恒星多重性函数的架构特征，特别是行星半径均匀性和周期间距的对数均匀性。

Result: 发现双星系统行星遵循与单星系统相似的"豌豆荚"模式，即行星半径趋向均匀、周期间距趋向对数均匀。但检测到中等显著性(2.5-3σ)的差异：双星系统具有更高的典型间隙复杂度（表明间距更不均匀）和更高的单行星系统比例。

Conclusion: 双星主要通过在原行星盘形成阶段塑造其结构来影响行星架构，随后的动力学过程在长期时间尺度上更温和地改变系统架构。这支持了双星在行星系统形成早期起主要作用的观点。
...
### [16] [Lava Lakes on Io: crust age and implications for thermal output](https://arxiv.org/abs/2603.22062)
...
TL;DR: 对木卫一火山熔岩湖热特性的研究显示，先前对其热功率的估计可能低估了10倍，主要热源来自低温地壳而非高温边缘环带


<details>
  <summary>Details</summary>
Motivation: Juno任务的JIRAM仪器观测证实木卫一许多火山热点是活跃的熔岩湖，需要研究这些熔岩湖的热特性以更好地理解其结构和能量预算

Method: 研究分析了30个熔岩湖的热特性，探索地壳平均温度与熔岩湖演化状态的关系，并提出改进的木卫一全球热输出评估方法

Result: 发现熔岩湖大部分总功率来自低温地壳而非高温边缘环带，先前估计可能低估了10倍；木卫一的paterae（火山口）在约十年时间尺度上经历随机重铺，每个湖可能遵循其特有的循环

Conclusion: 只有对木卫一进行具有足够空间和光谱分辨率的全表面观测，才能获得其火山总热通量的现实值，这对准确评估木卫一的全球热输出至关重要
...
### [17] [Direct spectroscopic confirmation of the young embedded proto-planet WISPIT 2c](https://arxiv.org/abs/2603.22085)
...
TL;DR: 在WISPIT 2系统中确认并表征了第二颗近距离行星WISPIT 2c，质量约为8-12 MJup，是之前确认的WISPIT 2b质量的两倍，使该系统成为研究多行星系统在原生盘中形成和早期演化的第二个实验室。


<details>
  <summary>Details</summary>
Motivation: WISPIT 2是一个拥有多环盘结构的年轻恒星系统，之前已确认存在一颗~4.9 MJup的气态巨行星。本研究旨在确认并表征该系统中的第二颗近距离行星，以提供研究多行星系统在原生盘中形成和早期演化的另一个重要案例。

Method: 结合新的VLT/SPHERE H波段双偏振成像和VLTI/GRAVITY K波段干涉观测数据。从GRAVITY数据中提取伴星的K波段光谱，使用species工具配合嵌套采样拟合大气模型网格，约束其有效温度、半径和光度。

Result: 确认了WISPIT 2c的存在，其有效温度为1500-2600 K，半径为0.91-2.2 RJup，光度为(-3.47)-(-3.63)。与演化轨迹比较得出质量范围为8-12 MJup，约为WISPIT 2b质量的两倍。天体测量排除了背景源的可能性，并初步检测到轨道运动。

Conclusion: WISPIT 2现在成为PDS 70的类似系统，为研究多行星系统在其原生盘内的形成和早期演化提供了第二个重要实验室。轨道运动需要进一步的后续观测来确认。
...
### [18] [QLP Data Release Notes 004: TESS-Gaia Light Curve Photometry Implementation](https://arxiv.org/abs/2603.22236)
...
TL;DR: QLP采用TESS-Gaia光变曲线包的测光方法改进测光精度，特别针对暗星（10.5 < T < 13.5），从第94扇区开始实施，提供更高精度的光变曲线和更深的行星搜索能力。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习方法实现更深的搜索和科学重点转向更暗的恒星，需要改进QLP在暗星亮度下的性能。原始QLP测光在10.5 < T < 13.5范围内噪声特性较差，需要提升测光精度。

Method: 采用TESS-Gaia光变曲线包（Han & Brandt 2023）的测光方法，该方法在10.5 < T < 13.5范围内比原始QLP测光具有更好的噪声特性。仍进行孔径测光，为所有亮于T=13.5的恒星提供3个孔径和3个级别的去趋势处理，确保外部用户无缝过渡。

Result: 该方法从第94扇区开始在QLP光变曲线中实施，为用户提供更高精度的光变曲线，并在行星搜索中能够检测到更暗的信号。

Conclusion: QLP通过采用改进的测光方法显著提升了暗星的光变曲线质量，支持更深入的行星搜索，同时保持了与现有用户工作流程的兼容性。
